"""
Standalone Streamlit UI for Streamlit Community Cloud.
Calls the LangGraph pipeline directly (no FastAPI backend required).
Identical look to the original app.py but with in-process pipeline call.
"""
...
.groq-badge {
  display: inline-block; background: #0f2027; border: 1px solid #f97316;
  color: #fdba74; padding: 3px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600;
}
</style>
...
# ── In-process pipeline call (no FastAPI required) ────────────────────────────
def run_pipeline(query: str, literacy_mode: str, session_id: str, conversation_history: list) -> dict:
    """Call the LangGraph pipeline directly (synchronously via asyncio.run)."""
    try:
        ...
        # Handle both cases: new event loop (Streamlit Cloud) or existing loop
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # Running inside an existing event loop (some Streamlit versions)
        ...
        return {"error": str(e), "final_answer": f"⚠️ Error: {e}"}
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='color:#9ca3af;font-size:0.82rem;margin-bottom:4px;'>"
        "GBM Research &amp; Care Intelligence</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<span class='groq-badge'>⚡ Powered by Groq — Free &amp; Fast</span>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    st.markdown("<span class='version-badge-b'>🅱️ Version B — NumPy hybrid RAG</span>", unsafe_allow_html=True)
...
<div style="text-align:center;padding:16px 0 8px;">
  ...
  <div style="color:#9ca3af;font-size:0.88rem;margin-top:6px;">
    GBM Research &amp; Care Intelligence &nbsp;·&nbsp; {_mode_badge} &nbsp;·&nbsp;
...
if query:
    ...
    st.markdown(
        ...
        f'Retrieval: <strong>NUMPY</strong></div>',
        unsafe_allow_html=True,
    )
    ...
    if "error" in result and "final_answer" not in result:
        bot_msg = {
            "role": "assistant",
...
def main():
    """Entry point for streamlit_app.py at root."""
    pass  # All Streamlit code runs at module level above
